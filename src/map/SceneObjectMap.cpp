...
SceneObjectMap::SceneObjectMap() : logger("SceneObjectMap") {}

void SceneObjectMap::loadObjectLayer(const tmx::ObjectGroup& layer) {
...
void SceneObjectMap::loadRectangleObjects(const tmx::Object &object) {
...
std::vector<std::shared_ptr<Collidable>>& SceneObjectMap::getMapCollidables() {
    return this->mapCollidables;
}

std::vector<Collidable> SceneObjectMap::getNpcCollidables() {
    return this->npcCollidables;
}

std::map<std::string, sf::IntRect> SceneObjectMap::getNpcMoveBoundariesMap() {
    return this->npcMoveBoundaries;
}

Collidable SceneObjectMap::getPlayerCollidable(std::string spawnName) {
    try {
...
std::map<std::string, std::string> SceneObjectMap::getNpcNameToNpcAssetNameMap() {
    return this->npcNameToNpcAssetNameMap;
}

std::string SceneObjectMap::getPlayerSpawnPointName(std::string sceneName) {
